...
if __name__ == "__main__":
    ...
    target = args.target
    if target is None:
        # draw all failure cases
        fig, axs = plt.subplots(1,2,figsize=(15,7))
        draw_socket(axs[0])
        draw_wall(axs[1])
        for i in range(8):
            GOAL = goalList[i]
            data_dir = os.path.join(sys.path[0],'data/office_room', policy+'_'+str(i))
            test_data = pd.read_csv(os.path.join(data_dir,"test_data.csv")).to_numpy()
            for j in range(len(test_data)):
                case = test_data[j]
                if not case[0]:
                    data = pd.read_csv(os.path.join(data_dir,"positions_"+str(j)+".csv")).to_numpy()
                    positions = (data[1,0]-GOAL[0], data[1,2]-GOAL[2])
                    axs[0].plot(positions[0],positions[1],color='r',marker='x')
                    x = case[1]-GOAL[0]
                    y = case[2]
                    a = case[4]
                    axs[1].plot(x,y,color='r',marker='x')
                    axs[1].plot([x,x+0.005*np.sin(a)],[y,y+0.005*np.cos(a)],color='r')
                    axs[1].plot(a,2.81,color='r',marker='+')

    else:
        GOAL = goalList[target]
        print("plot ", policy, target)

        data_dir = os.path.join(sys.path[0],'data/office_room', policy+'_'+str(target))
        test_data = pd.read_csv(os.path.join(data_dir,"test_data.csv")).to_numpy()

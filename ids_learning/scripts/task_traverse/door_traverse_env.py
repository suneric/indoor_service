...
class DoorTraverseEnv(DoorOpenEnv):
    def __init__(self,resolution=(64,64),cam_noise=0.0, door_width=0.9):
        super(DoorTraverseEnv, self).__init__(resolution, cam_noise, door_width)
        self.stage = 'pull'
        self.open = False
        self.delta = 0
        self.success = False
    ...
    def _set_init(self):
        ...
        while not self._random_init_mobile_robot():
    ...
    def _take_action(self, action_idx):
        ...
        print(action_idx, action, self.stage, self.success, self.fail)
    ...
    def _random_init_mobile_robot(self):
        ...
        if self.radref == None:
            ...
            radref = self.radref
        cx = 0.01*(radref[0]-0.5)+0.025
        cy = 0.01*(radref[1]-0.5)+self.door_dim[0]+0.04
        theta = 0.314*(radref[2]-0.5)+pi
        camera_pose = np.array([[cos(theta),sin(theta),0,cx],
                                [-sin(theta),cos(theta),0,cy],
                                [0,0,1,0.075],
                                [0,0,0,1]])
        mat = np.array([[1,0,0,0.49],
                        [0,1,0,-0.19],
                        [0,0,1,0],
                        [0,0,0,1]])
        ...
        robot_z = R[2,3]
        yaw = euler[2]
        return self._reset_mobile_robot(robot_x,robot_y,robot_z,yaw)

...
def dqn_train(env, num_episodes, max_steps, model_dir):
    ...
    model_dir = os.path.join(model_dir, "dqn", datetime.now().strftime("%Y-%m-%d-%H-%M"))
    summaryWriter = tf.summary.create_file_writer(model_dir)

    actor = jfv_actor_network(image_shape,force_dim,joint_dim,action_dim)
    buffer = JFVReplayBuffer(image_shape,force_dim,joint_dim,action_dim,capacity=10000,batch_size=64)
...
if __name__=="__main__":
    ...
    env.close()
    plot_episodic_returns("dqn train",ep_returns)

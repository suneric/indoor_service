...
class LatentDynamics(keras.Model):
    def __init__(self, latent_dim, action_dim, lr=1e-3,beta=1.0,**kwargs):
        super().__init__(**kwargs)
        self.beta = beta
...
class WorldModel(keras.Model):
    ...
    def train(self, buffer, epochs=200, batch_size=32, verbose=0, callbacks=None):
        size = buffer.size()
        images,forces,nimages,nforces,acts,rews,rets,advs,logps = buffer.get_data()
